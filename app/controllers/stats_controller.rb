class StatsController < ApplicationController
  ...
  def index
    ...
    sql = "SELECT to_char(event_date, 'yyyy-mm') as date, SUM(reputation_events.points) as count FROM reputation_events WHERE date_part('months', now() - reputation_events.event_date) < 12 AND reputation_events.user_id = %i GROUP BY date ORDER BY date;"
    ...
    @new_project_views = ProjectImpression.where(project_id: @user.projects.own.pluck(:id)).where("project_impressions.created_at > ?", Date.today.beginning_of_month).count
