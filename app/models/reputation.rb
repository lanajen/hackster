...
class Reputation < ActiveRecord::Base
  ...
  def compute
    ...
    self.points = user.live_visible_projects_count * 1.5 + user.followers_count * 2 + user.projects.live.own.map{|p| p.team_members_count > 0 ? (p.respects_count * 3 + Math.log(p.impressions_count)) / p.team_members_count : 0 }.sum
