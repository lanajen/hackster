class Embed
  LINK_REGEXP = {
    ...
    /(.+\.(jpg|jpeg|bmp|gif|png)(\?.*)?)$/ => { embed_class: 'original', position: '$1', code: '<img src="|id|" class="img-responsive">',},
    /(.+\.[^(html|htm|aspx|asp)](\?.*)?)$/ => { embed_class: 'original', position: '$1', code: '<div class="document-widget"><div class="file"><i class="fa fa-file-o fa-lg"></i><a href="|id|">|id|</a></div></div>',},
  }
  ...
  def self.find_code_for_url url
    ...
    LINK_REGEXP.each do |regexp, provider_code|
      if url =~ Regexp.new(regexp)
        ...
        when Hash
          id = eval provider_code[:position]
