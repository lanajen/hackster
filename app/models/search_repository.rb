...
#    query += ' ' + params[:tech_tag].keys.join(' ') if params[:tech_tag]
    results = self.search_models query, models, params[:offset], params[:page], params[:per_page]
#    Search.create(user_id: user_id, results_count: results.count, query: query) unless params[:page]
    results
  end

  protected
    def search_models query, models=nil, offset=nil, page=1, per_page=RESULTS_PER_PAGE
