...
import { fetchReviewThread, postDecision } from '../utils/ReactAPIUtils';
...
function requestThread(request) {
  return {
    type: REQUEST_THREAD,
    request
  };
...
function submitDecision(request, decision) {
  return {
...
function receiveThread(response) {
  return {
    type: RECEIVE_THREAD,
...
function fetchThreadFromServer(projectId) {
  return function (dispatch) {
    ...
    return promise
      .then(response =>
        dispatch(receiveThread(response.body))
